...
export const Route = createFileRoute('/_protected/dashboard/projects/$id')({
...
function ProjectDetailPage() {
  ...
  const { data: project, isLoading, isError, error } = useQuery({
    queryKey: ['project', id],
    queryFn: () => client.projects.getById(id),
    enabled: !!id,
  })

  if (isError) {
    return <DataTableErrorMessage resourceName="project" error={error ?? null} />
  }
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 pt-0">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground ml-12">Project details</p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <DetailRow label="Name" value={project.name ?? undefined} />
          <DetailRow label="Description" value={project.description ?? undefined} />
          <DetailRow
            label="Created"
            value={
              project.created_at
