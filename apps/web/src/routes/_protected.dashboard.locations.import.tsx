...
function parseCsv(text: string): Record<string, string>[] {
  ...
  const canonical = ['name', 'description', 'parent_location_id'] as const
  ...
  if (!mapKeys['parent_location_id']) {
    const p = rawHeaders.find((h) => /^(parentlocationid|parent_location_id|parent)$/.test(h))
    if (p) mapKeys[p] = 'parent_location_id'
  }
...
function LocationsImportPage() {
  ...
  const [rows, setRows] = React.useState<LocationImportRow[]>([])
  ...
  const runImport = React.useCallback(async () => {
    ...
    if (!activeTenantId || toImport.length === 0) {
      if (rows.length > 0) toast.error('Add at least one location with a name')
      return
    }
    setImporting(true)
    try {
  ...
  }, [activeTenantId, client.locations, queryClient, rows, navigate])
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 pt-0">
      <div className="space-y-1">
        <h2 className="text-lg font-semibold tracking-tight">Import locations</h2>
        <p className="text-muted-foreground text-sm">
          Upload a CSV or add rows below. Columns: <strong>name</strong> (required), description, parent_location_id (UUID). Rows with an empty name are skipped.
        </p>
      </div>
      ...
      {rows.length === 0 ? (
        ...
        <>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-t pt-4">
            <Link to="/dashboard/locations" className="text-muted-foreground text-sm hover:underline">
              Back to locations
            </Link>
            <Button onClick={runImport} disabled={importing || rowsWithName.length === 0}>
              {importing ? 'Importing…' : `Import ${rowsWithName.length} location${rowsWithName.length !== 1 ? 's' : ''}`}
            </Button>
          </div>
        </>
      )}
