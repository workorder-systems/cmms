...
5. Use work_orders_create to create a work order (this tool takes an explicit tenant_id). For automation/retries, include client_request_id. Before writes, restate intent and required fields; ask the user for missing/ambiguous inputs.
...
7. Prefer workflow_bundle first when you want a curated workflow. Otherwise prefer sdk_catalog_compact first. Call sdk_operation_schema only when you need the exact args schema. Use sdk_invoke for other operations.
