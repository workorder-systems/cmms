...
--
-- Migration: Bulk import work orders RPC
--
-- Purpose: Add rpc_bulk_import_work_orders so the UI can import many work orders
--   in one call with desired status/priority/due_date set on insert. Avoids
--   invalid status transitions (draft -> completed etc.) that occur when
--   creating then transitioning each row.
-- Affected: new function public.rpc_bulk_import_work_orders
-- Special: Input is jsonb array of { title, description?, status?, priority?, due_date? }.
--   Status and priority are validated against tenant catalogs; default status
--   is 'draft', default priority is 'medium'. For statuses that are final and
--   category 'closed', completed_at and completed_by are set on insert.
...
comment on function public.rpc_bulk_import_work_orders(uuid, jsonb) is
  'Bulk creates work orders for import. Accepts a jsonb array of rows with title (required), description, status, priority, due_date. Status and priority must exist in tenant catalogs; defaults: draft, medium. Sets completed_at/completed_by when status is final and category closed. Rate limited to 1 call per minute per user. Returns { created_ids: uuid[], errors: { index, message }[] }.';
