...
- **Projects** – list/get via `client.projects`; **create, update, delete** via `client.projects.create` / `update` / `delete` on the [Projects](/projects) page (`project.manage`); use **`projectId`** on work orders for roll-up
...
Projects group work orders for **cost roll-up**. From this app you typically **read** roll-ups with **`client.costs.listProjectCosts()`** or **`client.costs.costRollup({ groupBy: 'project', ... })`**.

**CRUD on project rows** (not read-only): use **`client.projects`** — see [Projects → Create project](/projects#create-project) for **`client.projects.create`**, plus update and delete on the same page. That requires **`project.manage`**.

When creating a work order, pass **`projectId`** so its costs appear in project roll-ups.

<CodeGroup title="Create a project, then attach a work order">
