...
The `client.costs` resource and `client.projects` resource support:
...
- **Projects** – list and get by ID (read-only); use `projectId` when creating work orders for cost roll-up by project
...
Projects are used to group work orders for cost roll-up. The SDK exposes read-only list and get by ID from `v_projects`. Create/update/delete projects via the database or future RPCs.

<CodeGroup title="client.projects.list()">

```ts
const projects = await client.projects.list()
// Returns ProjectRow[]: id, tenant_id, name, code, description, created_at, updated_at
```
...
When creating a work order, pass `projectId` to link it to a project so it appears in project cost roll-ups.
