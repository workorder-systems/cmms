...
Lists the catalog of available plugins (`v_plugins`) and tenant plugin installations (`v_plugin_installations`). Install, update, and uninstall plugins for a tenant. **Installing and managing installations requires `tenant.admin` permission.** Set tenant context before listing or managing installations. For plugin authors, see [Building plugins](/plugins-building). {{ className: 'lead' }}
