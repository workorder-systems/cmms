...
export const sections = [
  ...
  { title: 'Maintenance requests (pre-WO)', id: 'maintenance-requests' },
  { title: 'Communication log', id: 'comms-log' },
  ...
  { title: 'Coordinator SLA queue', id: 'sla-open-queue' },
]
...
flowchart LR
  ...
  P --> MR[maintenance_request submitted]
  MR --> WO[Work order + SLA due times]
...
## Maintenance requests (pre-WO)

For a **two-step** flow (request row first, work order when staff is ready), use **`rpc_create_maintenance_request`**. Use **`draft`** to capture intent without portal ABAC on locations (tenant/asset checks still apply); only **`submitted`** rows can be converted. Staff run **`rpc_convert_maintenance_request_to_work_order`** (**`workorder.edit`**). The usual portal button keeps using **`createRequest`**, which submits and converts in one step.

<CodeGroup title="client.workOrders.createMaintenanceRequest / convertMaintenanceRequestToWorkOrder">

```ts
const requestId = await client.workOrders.createMaintenanceRequest({
  tenantId,
  title: 'HVAC noise in lobby',
  status: 'submitted',
})
const workOrderId = await client.workOrders.convertMaintenanceRequestToWorkOrder(tenantId, requestId)

const allReqs = await client.workOrders.listMaintenanceRequests()
const mine = await client.workOrders.listMyMaintenanceRequests()
```
...
Append read-only communication events on a work order (**`v_work_order_comms`**) for coordinator and portal audit trails. **Requester**, **assignee**, or **`workorder.edit`** may append via **`rpc_add_work_order_comms_event`**.

<CodeGroup title="client.workOrders.addCommsEvent / listComms">

```ts
await client.workOrders.addCommsEvent({
  tenantId,
  workOrderId,
  body: 'Called tenant — awaiting access.',
  channel: 'phone',
})
const thread = await client.workOrders.listComms(workOrderId)
```
...
## Coordinator SLA queue

**`v_work_orders_sla_open`** lists **non-final** work orders that have SLA due times—useful for dispatch triage (minutes remaining, breach flags).

<CodeGroup title="client.workOrders.listSlaOpenQueue()">

```ts
const open = await client.workOrders.listSlaOpenQueue()
// WorkOrderSlaOpenRow[]
```
