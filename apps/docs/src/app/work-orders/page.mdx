export const metadata = {
  title: 'Work orders',
  description: 'List, create, transition, complete, log time, list attachments, update attachment metadata.',
}
...
<Properties>
  ...
  <Property name="projectId">
    Optional. Project UUID for cost roll-up by project. See [Costs & lifecycle](/costs).
  </Property>
</Properties>
...
Attachments use **Supabase Storage** and a **database trigger**: there is no RPC to “add” an attachment. You upload a file to the `attachments` bucket with a path that includes `tenant_id` and `work_order_id`. A trigger then creates the file record in `app.files` and the link in `app.work_order_attachments`. The SDK provides **list attachments** and **update attachment metadata**; use Storage for upload and signed URLs.

| Action | SDK / API |
|--------|-----------|
| Create attachment | `client.supabase.storage.from('attachments').upload(path, file, opts)` — path: `tenant_id/work_order_id/uuid_filename`; trigger creates records. |
...
<CodeGroup title="Raw: rpc_update_work_order_attachment_metadata">
