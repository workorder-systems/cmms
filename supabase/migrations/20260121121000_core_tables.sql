...
create index if not exists tenant_memberships_joined_at_brin_idx 
...
create index if not exists locations_parent_full_idx 
  on app.locations (parent_location_id);

create index if not exists locations_hierarchy_idx 
...
create unique index if not exists departments_tenant_code_unique_idx 
...
create or replace function util.validate_tenant_match(
  p_tenant_id uuid,
...
create or replace function util.validate_location_parent_tenant()
returns trigger
language plpgsql
security definer
set search_path = ''
...
create or replace function util.validate_location_circular_reference()
returns trigger
language plpgsql
security definer
set search_path = ''
...
create or replace function util.validate_asset_location_tenant()
returns trigger
language plpgsql
security definer
set search_path = ''
...
create or replace function util.validate_asset_department_tenant()
returns trigger
language plpgsql
security definer
set search_path = ''
...
create or replace function util.validate_work_order_asset_tenant()
returns trigger
language plpgsql
security definer
set search_path = ''
...
create or replace function util.validate_work_order_location_tenant()
returns trigger
language plpgsql
security definer
set search_path = ''
...
create index if not exists assets_tenant_location_idx 
  on app.assets (tenant_id, location_id);

create index if not exists assets_tenant_department_idx 
  on app.assets (tenant_id, department_id);

create unique index if not exists assets_tenant_asset_number_unique_idx 
...
create index if not exists assets_location_status_idx 
  on app.assets (tenant_id, location_id, status);

create index if not exists assets_listing_idx 
...
create index if not exists assets_covering_list_idx 
  on app.assets (tenant_id, location_id, status) 
  include (id, name, asset_number);

create index if not exists assets_fts_idx 
...
create index if not exists work_orders_tenant_status_updated_idx 
  on app.work_orders (tenant_id, status, updated_at desc);

create index if not exists work_orders_tenant_assigned_status_idx 
  on app.work_orders (tenant_id, assigned_to, status) 
  where assigned_to is not null;

create index if not exists work_orders_tenant_due_date_idx 
  on app.work_orders (tenant_id, due_date nulls last);

create index if not exists work_orders_assigned_to_idx 
